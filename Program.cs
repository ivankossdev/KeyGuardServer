    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            server.TelegramReceived += async (sender, e) =>
            {
                ...
                // ---- Детальная обработка событий (Cmd=0x80) ----
                if (header.Cmd == 0x80)
                {
                ...
                else if (header.Cmd == 0xF3 && header.Ident == 0x00 && header.Value == 0xF0)
                {
                ...
                // ---- Ответ на чтение (Cmd=0x90, Value=0xE2) ----
                else if (header.Cmd == 0x90 && header.Value == 0xE2)
                {
                ...
                else if ((header.Cmd == 0x83 || header.Cmd == 0x85) && header.Ident == 0x0F && header.Value == 0x73)
                {
                ...
                // ---- Подтверждение записи (Cmd=0x90, Value=0xE1) ----
                else if (header.Cmd == 0x90 && header.Value == 0xE1)
                {
                ...
                {
                    // Прочие команды просто логируем
                    Console.WriteLine($"  <- Другая команда: Cmd=0x{header.Cmd:X2}, Ident=0x{header.Ident:X2}, Value=0x{header.Value:X2}");
                }
            };
            ...
            Console.WriteLine("  state <serial>           - запросить состояние устройства (Cmd=0xF2)");
                ...
                if (completedTask == readTask)
                {
                    ...
                    switch (command)
                    {
                        case "state":
                            ...
                            Console.WriteLine($"  Отправка запроса состояния: {BitConverter.ToString(stateCmd)}");
                        ...
                        case "subscribe":
                            ...
                            Console.WriteLine($"  Отправка подписки: {BitConverter.ToString(subCmd)}");
